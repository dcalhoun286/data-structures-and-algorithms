...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => {
    res.send('Fun fact: my room looks like a jungle because I have so many plant babies!');
  });

  app.get('/favoritefoods', (req, res) => {
    res.send('Some of favorite foods: berries and fruit in general, Mexican, Thai');
  });
...
const count = (target, input) => {
  ...
  let flattenedArray = input.reduce(function (addItUp, whereWeAreAt) {
    return addItUp.concat(whereWeAreAt);
  }, []);

  let counter = 0;
  flattenedArray.filter(x => {
    if (x === target) {
      counter ++;
    }
  });
...
const totalSum = (input) => {
  ...
  /*
  I used the Array.prototype.reduce online MDN document to first put all of the numbers in one array ("flatten an array of arrays") and then I used the "sum all the values of an array" example to get the total sum
  https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce
  */

  const reducedArray = input.reduce(
    function(acc, val) {
  ...
  let sum = reducedArray.reduce(function (accum, curVal) {
    return accum + curVal;
  }, 0);
  return sum;
};
