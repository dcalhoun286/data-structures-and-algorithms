...
const toLastNames = people => {
  ...
  // console.log('full names array', fullNames);
  return fullNames;
};
...
const validatePin = (pin) => {
  ...
  Jason Quaglia pointed out that I was using .test incorrectly (I kept getting the message ".test is not a function")
  */
...
const validateEmail = (email) => {
  ...
  // Seamus B and I worked on this solution together with Bade H
...
const validatePhoneNumber = (phoneNumber) => {
  ...
  const regex = /(^\(\d{3}\) \d{3}-\d{4}$)|(^\(\d{3}\)\d{3} \d{4}$)|(^\d{3} \d{3}-\d{4}$)|(^\d{3}-\d{7}$)|(^\d{3}-\d{3} \d{4}$)|(^\d{3}-\d{3}-\d{4}$)|(^\d{3} \d{3} \d{4}$)|(^\d{6}-\d{4}$)|(^\d{9}$)|(^\d{3} \d{7})|(^\d{10}$)/;
