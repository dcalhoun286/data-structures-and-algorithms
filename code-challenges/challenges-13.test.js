...
const fixTheTypo = () => {
// Solution code here...
  // Documentation: I used this to understand how to use the .eq() selector
  const $li = $('#fruits').children('li').eq(2);
  $li.text('Pear');
  return $li;
};
...
const firstLetters = (arr) => {
  // Solution code here...
  const firstLettersArray = arr.map(str => {
    return str.substring(0,1);
  });
  console.log('first letters array', firstLettersArray);
...
const findHappiness = (arr) => {
  // Solution code here...
  const happinessArray = arr.filter(string => {
    if (string.includes(':)')) {
      return true;
    }
  });
